...
export function smith_waterman_algorithm(A, B, Match = 3, Dismatch = 3, Gap = 1) {
    const n = A.length;
    const m = B.length;
    const H = new Array(n + 1).fill(0).map(_ => new Array(m + 1).fill(0));
    let Mv = -1;
    let Mi = -1;
    let Mj = -1;
    for (let i = 1; i < n + 1; i++)
        for (let j = 1; j < m + 1; j++) {
            const v = H[i][j] = Math.max(H[i - 1][j - 1] + (A[i - 1] == B[j - 1] ? Match : -Dismatch), H[i - 1][j] - Gap, H[i][j - 1] - Gap, 0);
            if (v > Mv) {
                Mv = v;
                Mi = i;
                Mj = j;
...
export function string_similarity_by_smith_waterman_algorithm(A, B, Match = 3, Dismatch = 3, Gap = 1) {
...
export function string_correction_by_smith_waterman_algorithm_to_console(correctString, inputString, Match = 3, Dismatch = 3, Gap = 1) {
